...
#define SE05X_PK_EC_RW(pk) mbedtls_pk_ec_rw(pk)
#else
#define SE05X_PK_EC_RW(pk) mbedtls_pk_ec(pk)
#endif
...
int hcrypto_get_random(uint8_t *buffer, size_t bufferLen)
{
    ...
    esp_fill_random(buffer, bufferLen);
    return 0;
}
    ...
    uint8_t *key, size_t keylen, uint8_t *inData, size_t inDataLen, uint8_t *outSignature, size_t *outSignatureLen)
{
    ...
    if(cipher_info != NULL){
        ...
        if (ret == 0) {
            *outSignatureLen = mbedtls_cipher_info_get_block_size(cmac_cipher_ctx->cipher_info);
        }
    }
...
int hcrypto_cmac_final(void *cmac_cipher_ctx, uint8_t *outSignature, size_t *outSignatureLen)
{
    ...
    *outSignatureLen = mbedtls_cipher_info_get_block_size(cmac_ctx->cipher_info);
...
void *hcrypto_gen_eckey(uint16_t keylen)
{
    ...
    ret = mbedtls_ecp_gen_key(curve_id, SE05X_PK_EC_RW(*pKey), mbedtls_ctr_drbg_random, ctr_drbg);
    ENSURE_OR_GO_EXIT(ret == 0);
...
int hcrypto_sign_digest(void *key, const uint8_t *digest, size_t digestLen, uint8_t *signature, size_t *signatureLen)
{
    ...
    ret = mbedtls_pk_sign(
        pKey, MBEDTLS_MD_SHA256, digest, digestLen, signature, *signatureLen, signatureLen, mbedtls_ctr_drbg_random, ctr_drbg);
    ENSURE_OR_GO_EXIT(ret == 0);
